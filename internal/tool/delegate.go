...
func newDelegateTool(deps Dependencies) tool.Tool {
	desc := "Delegate to an expert. Use mode=sync for an immediate result, or mode=async to enqueue work for asynchronous execution."
...
func runDelegateTool(ctx context.Context, deps Dependencies, input delegateInput) (delegateOutput, error) {
	...
	return delegateOutput{Response: resp, Status: "succeeded"}, nil
}
...
func delegateMode(input delegateInput) string {
	...
	case "", "sync", "synchronous":
		return "sync"
	case "async", "asynchronous", "queued", "queue":
		return "async"
	default:
		return mode
	}
