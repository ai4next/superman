...
func TestRunDelegateToolDefaultsToSync(t *testing.T) {
	...
	if out.Response != "sync result" || out.Mode != "sync" || out.Status != "succeeded" {
...
func TestRunDelegateToolParallelEnqueuesTask(t *testing.T) {
	...
	if out.TaskID != "task-123" || out.Mode != "async" || out.Status != "queued" || out.Response != "" {
		t.Fatalf("output = %#v", out)
	}
